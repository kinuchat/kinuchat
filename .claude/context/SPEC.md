...
17. [Development Rules and Guidelines](#17-development-rules-and-guidelines)
...
## 17. Development Rules and Guidelines
...
│     ─────────────────────────────────                                        │
│     - If uncertain about architecture, tech stack, or approach: check spec  │
│     - If suggesting alternatives to what's specified: flag it explicitly    │
│     - If the spec is unclear: ask for clarification, don't assume           │
│     - Update the spec when decisions are made that change it                │
│                                                                              │
│  2. BUILD INCREMENTALLY                                                      │
│     ──────────────────────                                                   │
│     - Complete one component before starting the next                       │
│     - Verify code compiles after each significant change                    │
│     - Write tests alongside implementation, not after                       │
│     - Commit working code frequently with meaningful messages               │
│                                                                              │
│  3. TEST ON REAL DEVICES EARLY                                              │
│     ──────────────────────────────                                           │
│     - BLE does not work in simulators/emulators                             │
│     - Test on physical iOS and Android devices for mesh features            │
│     - Crypto performance differs significantly on real hardware             │
│                                                                              │
...
│     - Validate all inputs, especially from mesh peers                       │
│                                                                              │
│  5. REFERENCE SPECIFIC SECTIONS                                             │
│     ──────────────────────────────                                           │
│     - When implementing a feature, cite the relevant spec section           │
│     - Example: "Implementing Noise handshake per Section 6.2"               │
│     - This ensures alignment and makes review easier                        │
│                                                                              │
└─────────────────────────────────────────────────────────────────────────────┘
```

### 17.2 Platform-Specific Gotchas
...
├─────────────────────────────────────────────────────────────────────────────┤
│                                                                              │
│  BLE PERMISSIONS (must be set up early):                                    │
│  ────────────────────────────────────────                                    │
...
│  - Advertising slows to ~1/second in background                             │
│  - Connections may be dropped after ~30 seconds                             │
│  - Use state restoration for BLE central/peripheral                         │
│  - Test background behavior extensively on real devices                     │
│                                                                              │
│  KEYCHAIN:                                                                   │
│  ─────────                                                                   │
│  - Use kSecAttrAccessibleAfterFirstUnlock for keys                          │
│  - Keys are NOT available during device boot until first unlock             │
│  - Handle this gracefully in the identity service                           │
│                                                                              │
│  APP STORE:                                                                  │
│  ──────────                                                                  │
│  - Encryption requires export compliance documentation                      │
│  - BLE usage requires clear user-facing explanation                         │
│  - Background usage must be justified in review notes                       │
│                                                                              │
├─────────────────────────────────────────────────────────────────────────────┤
│  ANDROID GOTCHAS                                                             │
├─────────────────────────────────────────────────────────────────────────────┤
...
│  - ACCESS_FINE_LOCATION (required for BLE scanning on Android 6-11)         │
│  - ACCESS_COARSE_LOCATION (alternative for some use cases)                  │
│                                                                              │
│  FOREGROUND SERVICE:                                                         │
│  ────────────────────                                                        │
│  - Required for reliable background mesh operation                          │
│  - Must show persistent notification                                        │
│  - Use FOREGROUND_SERVICE permission                                        │
│  - Handle Doze mode and battery optimization                                │
│                                                                              │
│  DEVICE FRAGMENTATION:                                                       │
│  ──────────────────────                                                      │
│  - BLE behavior varies significantly across manufacturers                   │
│  - Samsung, Xiaomi, Huawei have aggressive battery optimization             │
│  - Some devices limit concurrent BLE connections                            │
│  - Test on at least 3-4 different manufacturers                             │
│                                                                              │
│  KEYSTORE:                                                                   │
│  ─────────                                                                   │
│  - Hardware-backed keystore not available on all devices                    │
│  - Fall back to software keystore gracefully                                │
│  - StrongBox (dedicated security chip) available on Pixel 3+                │
│                                                                              │
├─────────────────────────────────────────────────────────────────────────────┤
...
│  CRYPTO LIBRARIES:                                                           │
│  ─────────────────                                                           │
│  - pointycastle is pure Dart (slower but consistent)                        │
│  - cryptography package uses platform channels (faster but more setup)      │
│  - Test encryption/decryption on both platforms early                       │
│  - Benchmark performance with realistic message sizes                       │
│                                                                              │
│  BLE MTU:                                                                    │
│  ────────                                                                    │
│  - Default MTU is often 20-23 bytes                                         │
│  - Negotiate larger MTU (up to 512) for efficiency                          │
│  - iOS negotiates automatically, Android requires explicit request          │
│  - Always handle MTU smaller than expected                                  │
│                                                                              │
│  TIME SYNCHRONIZATION:                                                       │
│  ──────────────────────                                                      │
│  - Device clocks can be wrong                                               │
│  - Use relative time for TTL calculations in mesh                           │
│  - Don't rely on absolute timestamps for message ordering                   │
│  - Consider NTP sync when online                                            │
│                                                                              │
└─────────────────────────────────────────────────────────────────────────────┘
```
...
│     "I'm implementing [feature]. Per Section [X] of the spec..."            │
│                                                                              │
│  2. WRITE INTERFACES FIRST                                                  │
│     Define the contract before implementation                               │
│     Example: TransportManager interface before CloudTransport               │
│                                                                              │
│  3. IMPLEMENT WITH TESTS                                                    │
│     Write unit tests alongside the implementation                           │
│     Aim for >80% coverage on core/ and data/ layers                         │
│                                                                              │
│  4. VERIFY COMPILATION                                                      │
│     Run: melos run analyze && melos run test                                │
│     Fix all warnings, not just errors                                       │
│                                                                              │
│  5. TEST ON DEVICE (for platform features)                                  │
│     BLE, crypto, background execution must be tested on real devices        │
│                                                                              │
│  6. COMMIT WITH CONTEXT                                                     │
│     Good: "feat(mesh): implement Noise XX handshake per Section 6.2"        │
│     Bad: "add encryption"                                                   │
│                                                                              │
│  7. UPDATE SPEC IF NEEDED                                                   │
│     If implementation required changes from spec, update the spec           │
...
│                                                                              │
│  Scopes (examples):                                                          │
│  - core, mesh, crypto, transport, ui, matrix, relay, docs                   │
│                                                                              │
│  ═══════════════════════════════════════════════════════════════════════    │
│                                                                              │
│  WHEN STUCK:                                                                │
...
│  2. Check if there's a conflict between spec and implementation             │
│  3. Look at reference implementations (BitChat, Matrix SDK)                 │
│  4. Ask for clarification rather than guessing                              │
│  5. If spec is wrong or incomplete, propose an update                       │
│                                                                              │
└─────────────────────────────────────────────────────────────────────────────┘
```
...
│    Verify: flutter doctor (all checks should pass)                          │
│                                                                              │
│  □ Dart SDK (comes with Flutter)                                            │
│    Verify: dart --version (should be 3.x)                                   │
│                                                                              │
...
│                                                                              │
│  □ Rust toolchain (for relay server, Phase 4+)                              │
│    Install: curl --proto '=https' --tlsv1.2 -sSf https://sh.rustup.rs | sh │
│    Verify: rustc --version                                                  │
│                                                                              │
│  □ Physical test devices available                                          │
│    - At least one iOS device (iPhone 8 or newer recommended)                │
│    - At least one Android device (Android 8.0+)                             │
│    - Ideally 2+ of each for mesh testing                                    │
│                                                                              │
│  □ Git initialized                                                          │
...
│  BEFORE EACH PHASE:                                                          │
│  ──────────────────                                                          │
│  □ Previous phase code compiles without warnings                            │
│  □ Previous phase tests pass                                                │
│  □ Spec is up-to-date with any changes made                                 │
...
│  □ Tested on physical Android device (2+ manufacturers)                     │
│  □ Security review completed                                                │
│  □ Privacy policy updated                                                   │
│  □ App store assets prepared                                                │
│  □ Export compliance documentation ready (for encryption)                   │
│                                                                              │
└─────────────────────────────────────────────────────────────────────────────┘
```

### 17.5 Context Management for AI Assistants

```
┌─────────────────────────────────────────────────────────────────────────────┐
│  FOR AI ASSISTANTS (Claude Code CLI, etc.)                                   │
├─────────────────────────────────────────────────────────────────────────────┤
│                                                                              │
│  THIS SPEC IS LARGE. To manage context effectively:                         │
│                                                                              │
│  1. START EACH SESSION by reading relevant sections                         │
│     Don't try to hold the entire spec in context                            │
│     Focus on the current phase and feature                                  │
│                                                                              │
│  2. SECTION REFERENCE GUIDE:                                                │
│     ─────────────────────────                                                │
│     - Architecture questions     → Section 3                                │
│     - Tech stack questions       → Section 4                                │
│     - Feature requirements       → Section 5                                │
│     - Protocol/crypto details    → Section 6                                │
│     - UI/UX design               → Section 7                                │
│     - Onboarding flows           → Section 8                                │
│     - Server infrastructure      → Section 9                                │
│     - Cost/pricing               → Section 10                               │
│     - Security requirements      → Section 11                               │
│     - Marketing site             → Section 12                               │
│     - Donations                  → Section 13                               │
│     - Implementation timeline    → Section 14                               │
│     - CLI setup                  → Section 15                               │
│     - Glossary                   → Section 16                               │
│     - Dev rules (THIS SECTION)   → Section 17                               │
│                                                                              │
│  3. WHEN CONTEXT SEEMS LOST:                                                │
│     User can prompt: "Re-read Section [X] of the spec"                      │
│     This refreshes context on specific topics                               │
│                                                                              │
│  4. CITE YOUR SOURCES:                                                      │
│     When implementing, state: "Per Section X.Y of the spec, I'm..."         │
│     This confirms alignment and helps the user verify                       │
│                                                                              │
│  5. FLAG DEVIATIONS:                                                        │
│     If you need to deviate from the spec, explicitly say so:                │
│     "The spec says X, but I recommend Y because Z. Should I update spec?"   │
│                                                                              │
│  6. ASK BEFORE ASSUMING:                                                    │
│     If the spec doesn't cover something, ask:                               │
│     "The spec doesn't specify X. How would you like to handle it?"          │
